...
class DividedCorrelationConfig:
    ...
    def from_repo_root(
        ...
        nino4_monthly_path: Path | str | None = None,
        ...
        # Auto-detect Niño 4 path
        nino_path = Path(nino4_monthly_path) if nino4_monthly_path else _first_existing_path(
            [
                root / "data" / "index-monthly" / "nino4.anom.csv",
                project_data_dir / "index-monthly" / "nino4.anom.csv",
                *(
                    [climate_data_dir / "index-monthly" / "nino4.anom.csv"]
                    if climate_data_dir else []
                ),
            ],
            fallback=root / "data" / "index-monthly" / "nino4.anom.csv",
        )
...
def load_nino4_monthly(path: Path | str) -> pd.Series:
    csv_path = Path(path)
...
def run_workflow(config: DividedCorrelationV4Config) -> dict[str, object]:
    ...
    print(f"  Niño 4: {config.nino4_monthly_path}")
    ...
    nino_monthly = load_nino4_monthly(config.nino4_monthly_path)
