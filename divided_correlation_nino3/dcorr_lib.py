...
class DividedCorrelationConfig:
    ...
    def from_repo_root(
        ...
        nino3_monthly_path: Path | str | None = None,
        ...
        # Auto-detect Niño 3 path
        nino_path = Path(nino3_monthly_path) if nino3_monthly_path else _first_existing_path(
            [
                root / "data" / "index-monthly" / "nino3.anom.csv",
                project_data_dir / "index-monthly" / "nino3.anom.csv",
                *(
                    [climate_data_dir / "index-monthly" / "nino3.anom.csv"]
                    if climate_data_dir else []
                ),
            ],
            fallback=root / "data" / "index-monthly" / "nino3.anom.csv",
        )
...
def load_nino3_monthly(path: Path | str) -> pd.Series:
    csv_path = Path(path)
...
def plot_split_2x2(
    ...
    fig.suptitle(f"Split {split_id}: Curah Hujan DJF vs Niño 3", fontsize=12, y=0.98)
    save_png(fig, out_path, config.dpi)
...
def plot_hotspot_running_by_metric(
    ...
    metric_labels = {
        ...
        "nino_variance": "Varians Niño 3 Berjalan",
...
def run_workflow(config: DividedCorrelationV4Config) -> dict[str, object]:
    ...
    print(f"  Niño 3: {config.nino3_monthly_path}")
    ...
    nino_monthly = load_nino3_monthly(config.nino3_monthly_path)
