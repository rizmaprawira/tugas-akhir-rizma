#!/usr/bin/env python3
"""
Build derived DJF correlation datasets on a buffered regional domain.

The script computes the DJF seasonal means once on lon 80-180 / lat -40-40
with a 1-degree buffer, then computes correlations, p-values, and
significance masks against the Niño3.4 index. The resulting NetCDF files can
be reused when plotting extents change, so the expensive statistics do not
...
DJF_MONTHS = (12, 1, 2)
ANALYSIS_LAT_SLICE = slice(41, -41)
ANALYSIS_LON_SLICE = slice(79, 181)
...
DEFAULT_MEANS_NAME = "correlation_region_djf_means.nc"
DEFAULT_STATS_NAME = "correlation_region_djf_stats.nc"
...
def standardize_obj(obj):
    """Standardize time/lat/lon names, longitude convention, and chunking."""
    rename_map = {}
...
def main():
    ...
    print("[2/5] Building DJF seasonal means on the buffered regional domain...")
    ...
    # Keep the transport components on contiguous spatial chunks so the
    # latitude/longitude derivatives can be taken safely with dask.
    mfc_qx_monthly = mfc_qx_monthly.chunk({"time": 12, "lat": -1, "lon": -1})
    mfc_qy_monthly = mfc_qy_monthly.chunk({"time": 12, "lat": -1, "lon": -1})
    ...
    means_ds.attrs.update(
        {
            "title": "DJF seasonal means on the buffered regional analysis domain",
            "source_files": ", ".join(
                [
                    str(RAINFALL_PATH),
                    str(WIND_PATH),
    ...
    stats_ds.attrs.update(
        {
            "title": "DJF correlations and significance on the buffered regional analysis domain",
            "source_files": ", ".join(
                [
                    str(RAINFALL_PATH),
                    str(WIND_PATH),
