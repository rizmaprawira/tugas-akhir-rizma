...
- Detrending: removed; anomalies are used directly
- No standardization
...
class DividedCorrelationConfig:
    ...
    def from_repo_root(
        ...
        nino12_monthly_path: Path | str | None = None,
        ...
        # Auto-detect Niño 1+2 path
        nino_path = Path(nino12_monthly_path) if nino12_monthly_path else _first_existing_path(
            [
                root / "data" / "index-monthly" / "nina1.anom.csv",
                project_data_dir / "index-monthly" / "nina1.anom.csv",
                *(
                    [climate_data_dir / "index-monthly" / "nina1.anom.csv"]
                    if climate_data_dir else []
                ),
            ],
            fallback=root / "data" / "index-monthly" / "nina1.anom.csv",
        )
...
def load_nino12_monthly(path: Path | str) -> pd.Series:
    csv_path = Path(path)
...
def plot_split_2x2(
    ...
    fig.suptitle(f"Split {split_id}: Curah Hujan DJF vs Niño 1+2", fontsize=12, y=0.98)
    save_png(fig, out_path, config.dpi)
...
def plot_hotspot_running_by_metric(
    ...
    metric_labels = {
        ...
        "nino_variance": "Varians Niño 1+2 Berjalan",
...
def run_workflow(config: DividedCorrelationV4Config) -> dict[str, object]:
    ...
    print(f"  Niño 1+2: {config.nino12_monthly_path}")
    ...
    nino_monthly = load_nino12_monthly(config.nino12_monthly_path)
    ...
    djf_rain = djf_rain_anom
    djf_nino = djf_nino_anom

    print(f"  Climatology period: {config.clim_start}-{config.clim_end}")

    export_gridded_outputs(
        djf_rain.to_dataset(name="djf_rain_anom"),
        config.intermediate_dir / f"djf_rain_anom_{config.start_year}_{config.end_year}.nc",
    )
