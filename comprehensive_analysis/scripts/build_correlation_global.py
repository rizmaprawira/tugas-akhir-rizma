#!/usr/bin/env python3
"""
Build derived DJF correlation datasets on the full analysis domain.

The script computes the DJF seasonal means once, then computes correlations,
p-values, and significance masks against the Niño3.4 index. The resulting
NetCDF files can be reused when plotting extents change, so the expensive
statistics do not need to be recomputed.
"""
...
DEFAULT_MEANS_NAME = "correlation_global_djf_means.nc"
DEFAULT_STATS_NAME = "correlation_global_djf_stats.nc"
...
def main():
    ...
    print("[2/5] Building DJF seasonal means on the full domain...")
    ...
    means_ds.attrs.update(
        {
            "title": "DJF seasonal means on the full analysis domain",
            "source_files": ", ".join(
                [
                    str(RAINFALL_PATH),
                    str(WIND_PATH),
    ...
    stats_ds.attrs.update(
        {
            "title": "DJF correlations and significance on the full analysis domain",
            "source_files": ", ".join(
                [
                    str(RAINFALL_PATH),
                    str(WIND_PATH),
